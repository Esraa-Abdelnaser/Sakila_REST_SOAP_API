...
public class AddressMapperImpl implements AddressMapper {
    ...
    public Address partialUpdate(AddressDto addressDto, Address address) {
        ...
        if ( address.getStaffList() != null ) {
            List<Staff> list = staffDtoListToStaffList( addressDto.getStaffList() );
            if ( list != null ) {
                address.getStaffList().clear();
                address.getStaffList().addAll( list );
            }
        }
        else {
            List<Staff> list = staffDtoListToStaffList( addressDto.getStaffList() );
            if ( list != null ) {
                address.setStaffList( list );
            }
        }
        if ( address.getStoreList() != null ) {
            List<Store> list1 = storeDtoListToStoreList( addressDto.getStoreList() );
            if ( list1 != null ) {
                address.getStoreList().clear();
                address.getStoreList().addAll( list1 );
            }
        }
        else {
            List<Store> list1 = storeDtoListToStoreList( addressDto.getStoreList() );
            if ( list1 != null ) {
                address.setStoreList( list1 );
            }
        }
        if ( address.getCustomerList() != null ) {
            List<Customer> list2 = customerDtoListToCustomerList( addressDto.getCustomerList() );
            if ( list2 != null ) {
                address.getCustomerList().clear();
                address.getCustomerList().addAll( list2 );
            }
        }
        else {
            List<Customer> list2 = customerDtoListToCustomerList( addressDto.getCustomerList() );
            if ( list2 != null ) {
                address.setCustomerList( list2 );
            }
        }
