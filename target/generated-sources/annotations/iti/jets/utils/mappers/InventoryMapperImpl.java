...
public class InventoryMapperImpl implements InventoryMapper {
    ...
    public Inventory partialUpdate(InventoryDto inventoryDto, Inventory inventory) {
        ...
        if ( inventoryDto.getStore() != null ) {
            ...
            storeDtoToStore1( inventoryDto.getStore(), inventory.getStore() );
        }
        if ( inventoryDto.getLastUpdate() != null ) {
            inventory.setLastUpdate( inventoryDto.getLastUpdate() );
        }
        if ( inventory.getRentalList() != null ) {
            List<Rental> list = rentalDtoListToRentalList( inventoryDto.getRentalList() );
            if ( list != null ) {
                inventory.getRentalList().clear();
                inventory.getRentalList().addAll( list );
            }
        }
        else {
            List<Rental> list = rentalDtoListToRentalList( inventoryDto.getRentalList() );
            if ( list != null ) {
                inventory.setRentalList( list );
            }
        }
    ...
    protected List<Inventory> inventoryDtoListToInventoryList(List<InventoryDto> list) {
        ...
        for ( InventoryDto inventoryDto : list ) {
            list1.add( toEntity( inventoryDto ) );
        }
