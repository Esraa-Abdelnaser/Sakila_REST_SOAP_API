...
public class StoreMapperImpl implements StoreMapper {
    ...
    public Store partialUpdate(StoreDto storeDto, Store store) {
        ...
        if ( store.getStaffList() != null ) {
            ...
            if ( list != null ) {
                store.getStaffList().clear();
                store.getStaffList().addAll( list );
            }
        }
        else {
            List<Staff> list = staffDtoListToStaffList( storeDto.getStaffList() );
            if ( list != null ) {
                store.setStaffList( list );
            }
        }
        if ( store.getInventoryList() != null ) {
            List<Inventory> list1 = inventoryDtoListToInventoryList( storeDto.getInventoryList() );
            if ( list1 != null ) {
                store.getInventoryList().clear();
                store.getInventoryList().addAll( list1 );
            }
        }
        else {
            List<Inventory> list1 = inventoryDtoListToInventoryList( storeDto.getInventoryList() );
            if ( list1 != null ) {
                store.setInventoryList( list1 );
            }
        }
        if ( store.getCustomerList() != null ) {
            List<Customer> list2 = customerDtoListToCustomerList( storeDto.getCustomerList() );
            if ( list2 != null ) {
                store.getCustomerList().clear();
                store.getCustomerList().addAll( list2 );
            }
        }
        else {
            List<Customer> list2 = customerDtoListToCustomerList( storeDto.getCustomerList() );
            if ( list2 != null ) {
                store.setCustomerList( list2 );
            }
        }
    ...
    protected void staffDtoToStaff1(StaffDto staffDto, Staff mappingTarget) {
        ...
        if ( staffDto.getStore() != null ) {
            ...
            partialUpdate( staffDto.getStore(), mappingTarget.getStore() );
        }
        else {
